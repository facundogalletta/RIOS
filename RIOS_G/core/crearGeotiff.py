...
from core.interactive_windows import interactive_window, interactive_window_number,seleccionar_carpeta,seleccionar_imagen
...
if opcion == 'Seleccionar rutas':
    ...
    path_from_image='D:/fgalletta/Inspire2/Inspire 2_ANP/Descargando/Prueba2/'
    path_from_txt='D:/fgalletta/Inspire2/Inspire 2_ANP/Descargando/Prueba2/'
    path_to_GTIF='D:/fgalletta/Inspire2/Inspire 2_ANP/Descargando/Prueba2/'
    Mos='mos'
...
# Definir la ultima pendiente calibrada

pendiente_calibrada=-0.025#(22.36-22.18)/(65-238)+0.0001
fecha_pendiente_calibrada = '16/12/2020 - 14:00'

if correction_bordes==4:
    while True:
        tipo= input('''
                        #######################################################################################
                        # NO SE REALIZO CORRECCION DE BORDES ------>> SE DEBE DEFINIR PENDIENTE DE CORRECCION.#
                        #######################################################################################
                
                La correccion se realiza mediante una "pendiente", que consiste en un valor que se agrego como acumulacion de errores y se
                debe restar a cada imagen tantas veces como la cantidad de fotos que haya entre la foto inicial y esta:
                
                Imagen correcta = Imagen uncorrecta * ( 1-pendiente * ( N° foto-N° foto inicial) ) )
                
                La ultima pendiente calibrada corresponde al ''' + fecha_pendiente_calibrada + '''. ¿Desea definir una nueva pendiente?
                
                1. Usar pendiente calibrada actual.
                2. Definir nueva pendiente. En este caso puede ser util correr el codigo con el parametro test = FaLse.
                3. No corregir pendiente. Este punto es util si se desea calibrar una nueva pendiente.
        ...
        if tipo=='1':
            ...
            pendiente=input('''
                ...
                Ingrese el nuevo valor de la pendiente: ''')
...
# Ecepción para correr el script de georeferenciación, eto se usa si quiero ver el error acumulativo qur producen los efectos de borde.
...
if img_type==3:
    for i in range(3):
        Mos_i=path_to_GTIF+Mos+"_"+str(i)+".JPG"
    ...
    Mos_c=path_to_GTIF+Mos+".tif"
    for k in range(rangoFotosi,rangoFotosf+1):
        #Cargar la imagen a georeferenciar
        Prefix=prefijo(k)
        im=Image.open(path_from_image+"orthorectified_"+Prefix+str(k)+"_" + str(band)+".tif")
        imarray=np.array(im)
        
        # Determin
        if correction_bordes==4 and test==False:
            imarray=np.array(im)+pendiente*(k-rangoFotosi)
            imarray[np.where(imarray<17)]=0
        else:
            imarray=np.array(im)
        del im
        ...
        #Creación del ráster que contendrá el Geotiff:
        fn=path_to_GTIF+Prefix+str(k)+"_"+str(band)+"_georeferenced"+".tif"
    ...
    """------------ Armado de mosaico ------------"""
    files_to_mosaic = elem_mosaico
    g = gdal.Warp(Mos_c, files_to_mosaic, format="GTiff", options=["COMPRESS=LZW"], srcNodata=0, dstNodata=0)
    g = None