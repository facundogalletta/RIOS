...
from correction_distortion import calibration_raw,calibration_reflectance,calibration_thermal
from calibration_dji import calibration_dji
from find_corners import find_corners
from features import dron_features,camera_dji_features,camera_features


def georef_img(img_type, cam, height, width, FOVwidth, FOVheight, capture_img, pil_img, binary_img,band, csv_file, flag_features, DT_vignetting, correction_vignetting, panel_names, sky_names):
    
    """ Create a rectified image and auxiliaty parameters of georeferencing.
    ...
    if cam=='1':
        ...
        img = calibration_thermal(capture_img, band, height, width, DT_vignetting, correction_vignetting, panel_names, sky_names)
        img = np.array(Image.fromarray(img))
    ...
    x1_or, x2_or, x3_or, x4_or = find_corners(x, y, z, yaw, pitch0, roll0, FOVw, FOVh)
    x1, x2, x3, x4 = find_corners(x, y, z, yaw, pitch, roll, FOVw, FOVh)
    ...
    pt_upper_left = [np.dot(h21, np.array([x_min*fs, y_min*fs, 1]))[0] / np.dot(h21, np.array([x_min*fs, y_min*fs, 1]))[2],
                     np.dot(h21, np.array([x_min*fs, y_min*fs, 1]))[1] / np.dot(h21, np.array([x_min*fs, y_min*fs, 1]))[2]]
    
    return img, h12, h32, imReg, h_out, w_out, resHeight, resWidth, pt_upper_left, x1, x2, x3, x4, x_min, y_min, yawDeg, date_num, h_final, x, y, z
