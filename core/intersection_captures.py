...
def intersection(img_type, cam, path_img1, path_img2, FOVwidth, FOVheight, band, csv_file, flag_features, flag_interseccion,DT_vignetting, correction_vignetting, panel_names, sky_names):
    ...
    res_georef_1 = georef_img(img_type,cam,height, width, FOVwidth, FOVheight, capture_img1, pil_img1, binary_img1, band, csv_file, flag_features, DT_vignetting, correction_vignetting, panel_names,sky_names)
    ...
    res_georef_2 = georef_img(img_type,cam,height, width, FOVwidth, FOVheight, capture_img2, pil_img2, binary_img2,band, csv_file,flag_features,DT_vignetting,correction_vignetting,panel_names,sky_names)
    ...
    #####################################################
    ##-------------------------------------------------##
    ##-- Define water regions visibles as a polygons --##
    ##-------------------------------------------------##
    #####################################################
    ...
    for i in range(len(x_interseccion)):
        ...
        #########################################################################################################################
        ##---------------------------------------------------------------------------------------------------------------------##
        ##-- Calculate for the polygon intersection the pixel coordinates in the undistored image and in the rectified image --##
        ##---------------------------------------------------------------------------------------------------------------------##
        #########################################################################################################################
    ...
    if flag_interseccion==True:
        ...
        plt.plot(px_img1r[:,0],px_img1r[:,1],'b')
    ...
    return px_img1, px_img2 , px_img1r, px_img2r, bar1_pxp, bar2_pxp,im1Rec, im2Rec, img1_undist, img2_undist
